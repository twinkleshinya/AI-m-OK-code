...
v2.1 更新（优化版）：
  - 修复：Ollama prompt 增加"忠于原文"约束，防止标题与原文内容不一致
  - 修复：融资类新闻单独降权（-5分）并硬性限流（最多2条），解决融资新闻过多问题
  - 修复：新增语义标题去重（Jaccard相似度），过滤同一事件的重复报道
  - 优化：评分体系新增三大内容方向加分（AI工具/大模型/硬核科技各+5~6分）
  - 优化：BREAKING_KEYWORDS 剔除"融资|IPO|估值"，避免融资词汇虚增评分
  - 新增：FINANCING_KEYWORDS 独立正则，统一管理融资类内容的识别与限流
"""
...
def deduplicate_and_rank(all_items):
    ...
    # Step 3: 融资类新闻限流（最多保留 MAX_FINANCING 条）
    MAX_FINANCING = 2
    financing_count = 0
    final = []
    for item in unique:
...
def calculate_importance_score(item):
    ...
    # 8. Web3相关内容加分（确保Web3不被边缘化）
    if WEB3_KEYWORDS.search(text):
        score += 3

    # 9. ✅ 新增：三大方向内容倾向加分
    # 方向1：AI实用工具 & Web3（小红书/实操向）
...
核心原则：
1. 【忠于原文】title_zh 必须忠实反映原文核心事实，禁止夸大、脑补或添加原文没有的信息。
2. 【过滤融资】如果是纯融资/估值/投资新闻且无实质技术内容，ai_related 标记为 false。
3. 【过滤标题党】震惊/速看/99%的人/不转不是中国人等情绪化词汇——直接过滤，ai_related=false。
4. 【三个内容方向优先】优先保留以下类型：
   - AI实用工具教程（绘画/办公/证件照/写真等实操内容）
   - 大模型重大进展（发布/降价/开源/能力突破/监管政策）
   - 硬核技术分析（AI芯片/具身智能/机器人/模型原理拆解）
   - Web3/区块链行业动向

每条必须包含以下字段：
- ai_related: true或false（判断是否真正与AI直接相关，纯融资/标题党为false）
- emoji: 一个贴切的emoji
- title_zh: 优化后的中文标题（15-25字，必须忠于原文事实，简洁有力，禁止夸大）
...
差标题（禁止出现）：「震惊！XX公司融资X亿，AI赛道再迎独角兽」→ 应过滤
...
核心原则：
1. 【忠于原文】title_zh 必须忠实反映原文标题的核心事实，绝对不能添加原文没有的信息或夸大描述。
2. 【过滤标准】以下情况 ai_related 标记为 false：
   - 纯融资/估值/投资轮新闻，且无实质技术/产品内容
   - 与AI/人工智能无直接关联的普通科技/硬件新闻
   - 含情绪化标题党词汇（shocking/you won't believe等）
3. 【内容偏好】以下类型优先保留（ai_related=true）：
   - LLM大模型最新进展（DeepSeek/OpenAI/Claude/Gemini等发布、降价、开源）
   - AI实用工具与应用场景（办公/创作/绘画/Agent）
   - AI芯片、具身智能、机器人
   - Web3/区块链行业动向
   - AI政策、监管、行业格局变化
...
- emoji: 一个贴切的emoji
- title_zh: 中文标题（必须是中文！15-25字，忠于原文事实，不要直译但不要夸大）
- summary_zh: 中文摘要（必须是中文！50-100字，解释这件事为什么重要，有洞察力）

好标题：「OpenAI 关停 Sora：日烧百万美元，用户不到50万」（忠于原文数据）
差标题：「震惊！OpenAI 巨额亏损，AI视频彻底凉了」（夸大+标题党，禁止）

注意：title_zh 和 summary_zh 都必须是中文！
