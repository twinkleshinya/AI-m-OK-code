...
摘要生成: Google Gemini API (gemini-2.0-flash)，免费额度充足。

v2.2 更新（Gemini 版）：
  - 重构：将 Ollama 本地模型替换为 Gemini API，支持云端部署
  - 新增：Gemini 原生 JSON 模式（responseMimeType），输出更稳定
  - 新增：速率控制（免费版 15 RPM），自动等待与重试
  - 新增：安全设置放宽，避免新闻内容被误拦截
...
# 🔄 CHANGED: Gemini API 配置（替换 Ollama）
GEMINI_API_KEY = os.environ.get("GEMINI_API_KEY", "AIzaSyAwesMzAFIU45qjxw0ISW92L-ufU4tFG78")
GEMINI_MODEL = "gemini-2.0-flash"                # 推荐：速度快、免费额度高、中文好
GEMINI_URL = f"https://generativelanguage.googleapis.com/v1beta/models/{GEMINI_MODEL}:generateContent"
GEMINI_RPM_DELAY = 4                             # 免费版 15 RPM → 每条间隔 4 秒
...
def fetch_tldr():
    ...
    try:
        ...
        # 第一步：提取所有外部链接（排除 tldr.tech 自身链接和广告）
...
# ══════════════════════════════════════════════════════
# 🔄 CHANGED: Gemini API 调用（替换 Ollama）
# ══════════════════════════════════════════════════════


def generate_chinese_summaries(items):
    """用 Gemini API 为每条资讯生成编辑式中文标题和摘要。"""
...
# 🔄 CHANGED: 完整替换 _call_ollama_single → _call_gemini_single
def _call_gemini_single(prompt: str, timeout: int = 60) -> str:
    """调用 Gemini API，返回原始文本，失败时返回空字符串。"""
    if not GEMINI_API_KEY:
        logger.warning("⚠️ GEMINI_API_KEY not set! Please set environment variable.")
        return ""
    ...
    payload = {
        ...
        "generationConfig": {
            ...
            # 🔄 CHANGED: Gemini 原生 JSON 模式，输出更稳定
            "responseMimeType": "application/json",
        },
        # 🔄 CHANGED: 安全设置放宽，避免新闻内容被误拦截
        "safetySettings": [
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if resp.status_code == 429:
                ...
                logger.warning(f"Gemini rate limited (429), waiting {wait_time}s... (attempt {attempt + 1}/{max_retries + 1})")
            ...
            if not candidates:
                # 可能被 safety filter 拦截
                block_reason = data.get("promptFeedback", {}).get("blockReason", "")
                if block_reason:
                    logger.warning(f"Gemini blocked by safety filter: {block_reason}")
                else:
                    logger.warning(f"Gemini empty candidates: {json.dumps(data, ensure_ascii=False)[:300]}")
                return ""
            ...
            if finish_reason == "SAFETY":
                logger.warning("Gemini response blocked by safety filter (finishReason=SAFETY)")
                return ""

            logger.warning(f"Gemini empty text in response: {json.dumps(data, ensure_ascii=False)[:300]}")
            return ""

        except requests.exceptions.Timeout:
            logger.warning(f"Gemini call timed out ({timeout}s), attempt {attempt + 1}/{max_retries + 1}")
...
def _parse_single_json(text: str) -> dict:
    ...
    # Gemini JSON 模式通常直接返回合法 JSON，但保留容错逻辑
    # 先尝试直接解析整段文本
    try:
...
def _process_item(item: dict, is_chinese: bool) -> dict | None:
    """用 Gemini 逐条处理单条资讯，返回处理后的 item 或 None（过滤掉）。"""
    title = item["title"]
    summary = item.get("summary", "")
    source = item["source"]
    ...
    # 🔄 CHANGED: 调用 Gemini 替代 Ollama
    text_out = _call_gemini_single(prompt, timeout=60)

    # 🔄 CHANGED: 速率控制（Gemini 免费版 15 RPM）
...
"""
    # 🔄 CHANGED: 调用 Gemini 替代 Ollama
    text_out = _call_gemini_single(prompt, timeout=45)
    result = _parse_single_json(text_out)
    if result:
        cn = result.get("cn", "")
...
def build_feishu_card(items, date_str):
    ...
    elements.append({
        "tag": "note",
        "elements": [{"tag": "plain_text", "content": f"🥕 AI-Scream 自动生成 | {date_str} | Powered by Gemini"}],
    })
...
def main():
    beijing_tz = timezone(timedelta(hours=8))
    today = datetime.now(beijing_tz).strftime("%Y-%m-%d")
    logger.info(f"=== AI-Scream Gemini Edition v2.2 | {today} ===")

    # 🔄 CHANGED: 启动时检查 Gemini API Key
    if not GEMINI_API_KEY:
        logger.error("❌ GEMINI_API_KEY not set! Please set environment variable: export GEMINI_API_KEY='your-key'")
    ...
    # 3. 生成中文标题与摘要（🔄 CHANGED: 使用 Gemini）
    logger.info("[9/9] Generating Chinese summaries via Gemini...")
